...
dqn = DQN(qnet, qnet_t,
          ...
          discount=0.99, learning_rate=0.01,
          epsilon_decay=0.999996, lr_scheduler=torch.optim.lr_scheduler.StepLR,
          lr_scheduler_kwargs=[{"step_size": 1000}])
...
for j in range(num_eps):
    ...
    while not done:
        ...
        dqn.store_transition({
            ...
            "reward": float(norm_factor*reward),
            "terminal": done
        })
        state = new_state

    if j % 10 == 0 and j != 0:
        print("reward: ", np.mean(rew))
        rew = []
...
print(rew)
print(np.mean(rew_m_l), print(np.std(rew_m_l)))

print(cost_m_l)
print(np.mean(cost_m_l), print(np.std(cost_m_l)))
