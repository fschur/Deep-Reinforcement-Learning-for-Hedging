    ...
    for i in range(n):
        ...
        for tupel in D:
            ...
            loss = torch.pow((tupel["nop"] - tupel["op"]) - out * (tupel["np"] - tupel["p"]), 2) + \
                   (T / num_steps) * trading_costs * tc_para
            ...
            delta_loss = ((tupel["nop"] - tupel["op"]) - tupel["delta"] * (tupel["np"] - tupel["p"]))**2 + \
                         (T / num_steps) * trading_costs_delta * tc_para
    ...
    for _ in range(10):
        ...
        for tupel in D:
            ...
            loss += norm_factor*(torch.pow((tupel["nop"] - tupel["op"]) - out * (tupel["np"] - tupel["p"]), 2) + \
                    (T/num_steps) * tc_para * trading_costs)
            old_out = out.detach().numpy()[0]
...
plt.xlabel("loss")
plt.legend()
plt.savefig("test_deep_classic_new_no_delta_losses_final_new_2_cost.png")

torch.save(model, "model_classic_final_new_2_cost.pth")
...
for i in range(5):
    ...
    plt.xlabel("time")
    #plt.xlabel("")
    plt.savefig("test_deep_classic_new_no_delta_final_new_2_cost" + str(i) + ".png")
    plt.show()
    plt.clf()
    plt.cla()
    plt.close()
...
def test_2(model, apm, opm, num_steps, dt, n=10):
    ...
    for tupel in D:
        ...
        delta_loss = (-tupel["nop"] + tupel["op"]) + tupel["delta"] * (tupel["np"] - tupel["p"]) - \
                     (T/num_steps) * tc_para * trading_costs_delta
        delta_losses.append(delta_loss)
