...
def test(model, apm, opm, num_steps, dt, n=10):
    ...
    for i in range(n):
        ...
        for tupel in D:
            ...
            delta_loss = (pl_delta) ** 2 - 1/100 *pl_delta
...
for i in range(num_eps):
    ...
    for _ in range(10):
        ...
        for tupel in D:
            ...
            trading_costs = (T / num_steps) * (abs(old_out - out) + 0.01 * (old_out - out) ** 2)
            pl = (-tupel["nop"] + tupel["op"]) + out * (tupel["np"] - tupel["p"]) - trading_costs
            loss += norm_factor * (torch.pow(pl, 2) - 1 / 100 * pl)
            old_out = out.detach().numpy()[0]
...
plt.xlabel("loss")
plt.legend()
plt.savefig("test_deep_classic_new_no_delta_losses_final_new_2_cost_100.png")

torch.save(model, "model_classic_final_new_2_cost_100.pth")
...
for i in range(5):
    ...
    plt.xlabel("time")
    # plt.xlabel("")
    plt.savefig("test_deep_classic_new_no_delta_final_new_2_cost_100" + str(i) + ".png")
    plt.show()
    plt.clf()
    plt.cla()
    plt.close()
