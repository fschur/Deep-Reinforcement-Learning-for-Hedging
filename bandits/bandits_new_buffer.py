...
num_actions = 21
model = space_bandits.NeuralBandits(num_actions, 3, layer_sizes=[20, 20], initial_pulls=20, lr_decay_rate=0.99,
                                    reset_lr=False, do_scaling=True, buffer_s=50000, memory_size=50000,
                                    initial_lr=0.1, training_freq=1,
                                    training_freq_network=50, training_epochs=100)
...
def test(model, apm, opm, num_steps, dt):
    ...
    delta_rewards = []
    for tupel in D:
        inp = np.array([[tupel["delta"], tupel["p"], tupel["ttm"]]], dtype=np.float)
