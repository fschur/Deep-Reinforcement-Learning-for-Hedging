...
for i in range(num_eps):
    ...
    for tupel in D:
        ...
        action = out / num_actions
        pl = (-tupel["nop"] + tupel["op"]) + action * (tupel["np"] - tupel["p"])
        reward = norm_factor*(-(pl)**2 + 1/100*pl)
        rewards.append(reward)
...
for i in range(8):
    ...
    plt.savefig("bandit_100" + str(i) + ".png")
    plt.clf()
    plt.cla()
    plt.close()
...
for i in range(8):
    ...
    plt.savefig("bandit_no_100" + str(i) + ".png")
    plt.clf()
    plt.cla()
    plt.close()
